...
export default function AgentDetails() {
  ...
  const currentDiskUsage = agent?.latest_snapshot?.data.disk_space.usage_percent ?? 0;
  ...
  const diskUsedTimestamps = agent?.history
  ...
  const cpuAvgTimestamps = agent?.history
    .map((snap) => snap.timestamp)
  ...
  return (
    <div className="px-6 py-6 flex-1 overflow-y-auto">
      ...
      <div className="mb-6 flex flex-row justify-start gap-6">
        <div className="w-64">
          <DiskGauge value={currentDiskUsage} label="Disk Usage" total={formatByteStr(currentDiskTotal)} used={formatByteStr(currentDiskUsed)} />
        </div>
