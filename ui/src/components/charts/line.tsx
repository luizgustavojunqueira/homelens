...
const color = (index: number, total: number) =>
  `hsl(${((index + 1) * 360) / Math.max(total + 1, 1)}, 65%, 55%)`;
...
>
  ...
  <span>${escapeHtml(valueFormatter(summary))}</span>
</div>
`
    : ""
}
...
<div
  ...
  "
>
<span style="color:${escapeHtml(serieColor)}">
  <span
    style="
      ...
      height:10px;
      border-radius:50%;
      background:${escapeHtml(serieColor)};
      margin-right:6px;
  ...
  <span>
    ${escapeHtml(valueFormatter(value))}
  </span>
</div>
`;
    })
...
`;
  ...
  }, [timestamps, series, valueFormatter, label, isTotalAverage]);
